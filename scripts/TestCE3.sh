#!/bin/bash
#  Shellscript to create test suites for Test CE3 (Commanding Engineering 3).
#  Check all possible commands with No Ack response. Verify the correct reception of all the data associated to the 
...
workDir=$HOME/LSST/robotframework_TMA/OPERATION_MANAGER/Test_CE3
...
function createSettings() {
  ...
  echo "Documentation    Execute ${command^^} command and get NoAck response." >> $testSuite
  echo "Library    SSHLibrary" >> $testSuite
  echo "Library    String" >> $testSuite
  echo "Resource    ../../Global_Vars.robot" >> $testSuite
  echo "Force Tags    Test_CE3" >> $testSuite
	echo "" >> $testSuite
}
...
function readNoAck() {
  echo "Read NoAck" >> $testSuite
  echo "    [Documentation]    Verify that EUI receives NoAck message sent from PXI." >> $testSuite
  echo "    [Tags]    functional" >> $testSuite
  echo "    Switch Connection    Netcat" >> $testSuite
  echo "    \${output}=    Read Until Regexp    (?m)^1\\\\s+[1-9][0-9]*\\\\s+2\\\\s+[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\\\\.[0-9]{6}" >> $testSuite
  echo "    Log    \${output}" >> $testSuite
  echo "" >> $testSuite
}
