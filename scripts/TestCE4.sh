#!/bin/bash
#  Shellscript to create test suites for Test CE4 (Commanding Engineering 4).
#  Check all possible commands with Ack Response and a Timeout situation before receiving the Done/Error response. 
#  For all commands, check the situation in which the Done/Error response arrives after the timeout. Verify the correct 
#  execution of the Engineering state machine. 
#
...
workDir=$HOME/LSST/robotframework_TMA/OPERATION_MANAGER/Test_CE4

# Array with command names
declare -a commandArray=(hmi_move_to_target hmi_stop az_axis_power az_axis_stop az_axis_move \
az_axis_home az_axis_reset_alarm az_drive_reset az_drive_enable \
az_cw_power az_cw_stop az_cw_move az_cw_track_azimuth az_cw_reset_alarm az_cw_drive_reset \
az_cw_drive_enable el_axis_power el_axis_stop el_axis_move el_axis_home \
...
mc_stop mc_move mc_open mc_close mc_reset_alarm cam_cw_power cam_cw_stop cam_cw_move \
cam_cw_track_camera cam_cw_reset_alarm cam_cw_drive_enable cam_cw_drive_reset bal_power \
bal_stop bal_move bal_reset_alarm tec_power tec_track_ambient tec_reset_alarm dp_power dp_stop \
dp_reset_alarm cabinet_track_ambient cabinet_reset_alarm lp_power lp_stop lp_move lp_reset_alarm \
...
declare -a commandIdArray=(30 32 101 102 103 106 107 201 202 301 302 303 305 306 307 308 401 402 403 \
406 407 501 502 601 602 701 702 703 801 802 803 804 805 901 902 903 905 906 907 1001 1002 1003 1004 1005 1006 \
1007 1101 1102 1103 1104 2201 2202 2203 1201 1202 1205 1301 1302 1401 1402 1403 1405 1501 1502 1503 \
...
function createSettings() {
  ...
  echo "Force Tags    Test_CE4" >> $testSuite
	echo "" >> $testSuite
}
...
function verifyTimedout() {
  echo "Verify Timed Out" >> $testSuite
  echo "    [Documentation]    Verify that Operation Manager times out waiting for Done-Error response." >> $testSuite
  echo "    [Tags]    functional" >> $testSuite
  echo "    Switch Connection    OperationManager" >> $testSuite
  echo "    \${output}=    Read Until    Timed out waiting for Done response for command" >> $testSuite
  echo "    Log    \${output}" >> $testSuite
  echo "    Should Contain    \${output}    Timed out waiting for Done response for command" >> $testSuite
  echo "" >> $testSuite
}
