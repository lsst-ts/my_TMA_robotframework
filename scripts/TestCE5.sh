#!/bin/bash
#  Shellscript to create test suites for Test CE5 (Commanding Engineering 5).
#  Check the reception of a new command when the state machine is waiting for the Done/Error response of a previously 
#  sent command. Verify the correct execution of the Engineering state machine. 
#
...
workDir=$HOME/LSST/robotframework_TMA/OPERATION_MANAGER/Test_CE5

# Array with command names
declare -a commandArray=(hmi_move_to_target az_axis_power az_axis_move \
az_axis_move_velocity az_axis_home az_axis_reset_alarm az_drive_reset az_drive_enable \
az_cw_power az_cw_move az_cw_move_velocity az_cw_track_azimuth az_cw_reset_alarm az_cw_drive_reset \
az_cw_drive_enable el_axis_power el_axis_move el_axis_move_velocity el_axis_home \
...
mc_move mc_move_velocity mc_open mc_close mc_reset_alarm cam_cw_power cam_cw_move \
cam_cw_track_camera cam_cw_reset_alarm cam_cw_drive_enable cam_cw_drive_reset cam_cw_move_velocity bal_power \
bal_move bal_reset_alarm tec_power tec_track_ambient tec_reset_alarm dp_power dp_move_velocity \
dp_reset_alarm cabinet_track_ambient cabinet_reset_alarm lp_power lp_move lp_move_velocity lp_reset_alarm \
mcl_power mcl_move mcl_reset_alarm az_thermal_power az_thermal_track_ambient az_thermal_reset_alarm \
...
declare -a commandIdArray=(30 101 103 104 106 107 201 202 301 303 304 305 306 307 308 401 403 \
404 406 407 501 502 601 602 701 702 703 801 802 803 804 805 901 903 904 905 906 907 1001 1003 1004 1005 1006 \
1007 1008 1101 1103 1104 2201 2202 2203 1201 1204 1205 1301 1302 1401 1403 1404 1405 1501 1503 \
...
function createSettings() {
  ...
  echo "Documentation    Execute ${command^^} command and get Ack/Done responses." >> $testSuite
  echo "Library    SSHLibrary" >> $testSuite
  echo "Library    String" >> $testSuite
  echo "Resource    ../../Global_Vars.robot" >> $testSuite
  echo "Force Tags    Test_CE5" >> $testSuite
	echo "" >> $testSuite
}
...
function sendStopCommand() {
  ...
  if [ "$command" == "az_axis_move_velocity" ]; then
    stop_command=102
  elif [ "$command" == "az_cw_move_velocity" ]; then
    stop_command=302
  elif [ "$command" == "el_axis_move_velocity" ]; then
    stop_command=402
  elif [ "$command" == "mc_move_velocity" ]; then
    stop_command=902
  elif [ "$command" == "cam_cw_move_velocity" ]; then
    stop_command=1002
  elif [ "$command" == "dp_move_velocity" ]; then
    stop_command=1202
  elif [ "$command" == "lp_move_velocity" ]; then
    stop_command=1402
  fi
    
  echo "Send Stop Command" >> $testSuite
  echo "    [Documentation]    Send stop for the ${command^^} command." >> $testSuite
  echo "    [Tags]    functional" >> $testSuite
  echo "    Switch Connection    Eui" >> $testSuite
  echo "    Comment    Enter command id" >> $testSuite
  echo "    Write    ${stop_command}" >> $testSuite
  # Depending on the command, insert the corresponding parameters
  if [ "$command" == "mc_move_velocity" ] || [ "$command" == "dp_move_velocity" ] || [ "$command" == "lp_move_velocity" ]; then
    getIdent
    echo "    \${output}=    Read Until    Enter drive ident:" >> $testSuite
    echo "    Write    1" >> $testSuite
  fi
...
function readDone() {
  echo "Read Done" >> $testSuite
  echo "    [Documentation]    Verify that EUI receives Done message sent from PXI." >> $testSuite
  echo "    [Tags]    functional" >> $testSuite
  echo "    Switch Connection    Netcat" >> $testSuite
  echo "    \${output}=    Read Until Regexp    (?m)^2\\\\s+[1-9][0-9]*\\\\s+2\\\\s+[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\\\\.[0-9]{6}" >> $testSuite
  echo "    Log    \${output}" >> $testSuite
  echo "" >> $testSuite
}
...
function verifyCommandRejected() {
  echo "Verify Command Rejected" >> $testSuite
  echo "    [Documentation]    Verify that Operation Manager rejects the command." >> $testSuite
  echo "    [Tags]    functional" >> $testSuite
  echo "    Switch Connection    OperationManager" >> $testSuite
  echo "    \${output}=    Read Until    Command execution not allowed in current state" >> $testSuite
  echo "    Log    \${output}" >> $testSuite
  echo "    Should Contain    \${output}    Command execution not allowed in current state" >> $testSuite
  echo "" >> $testSuite
}
...
function createTestSuite() {
  ...
  sendCommand "Send New Command"
  verifyCommandRejected
  if [[ ${commandGroup4[*]} =~ $command ]] || [[ ${commandGroup9[*]} =~ $command ]]; then
    readDone
  elif [[ ${commandGroup5[*]} =~ $command ]] || [[ ${commandGroup10[*]} =~ $command ]]; then
    sendStopCommand
  else
    readDone
  fi
