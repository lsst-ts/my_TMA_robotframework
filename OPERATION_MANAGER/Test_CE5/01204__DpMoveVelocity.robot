*** Settings ***
Documentation    Execute DP_MOVE_VELOCITY command and get Ack/Done responses.
Library    SSHLibrary
Library    String
...
Verify PXI Message
    ...
    Should Match Regexp    ${output}    [1-9][0-9]*-[1-9][0-9]*-1204-2-[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}-4-2\\.895015-2\\.001235-19\\.849292
...
Send Stop Command
    [Documentation]    Send stop for the DP_MOVE_VELOCITY command.
    [Tags]    functional
    Switch Connection    Eui
    Comment    Enter command id
    Write    1202
