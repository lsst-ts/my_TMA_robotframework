*** Settings ***
Documentation    Execute MC_MOVE_VELOCITY command and get Ack/Done responses.
Library    SSHLibrary
Library    String
...
Verify PXI Message
    ...
    Should Match Regexp    ${output}    [1-9][0-9]*-[1-9][0-9]*-904-2-[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}-2-9\\.290388-2\\.025522-28\\.746436
...
Send Stop Command
    [Documentation]    Send stop for the MC_MOVE_VELOCITY command.
    [Tags]    functional
    Switch Connection    Eui
    Comment    Enter command id
    Write    902
