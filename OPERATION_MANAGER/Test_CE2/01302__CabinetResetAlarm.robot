*** Settings ***
Documentation    Execute CABINET_RESET_ALARM command and get Ack/Error responses.
Library    SSHLibrary
Library    String
...
*** Test Cases ***
Send Command
    [Documentation]    Send the CABINET_RESET_ALARM command.
    [Tags]    functional
...
Verify Operation Manager Dispatches Command
    [Documentation]    Verify that Operation Manager dispatches the CABINET_RESET_ALARM command.
    [Tags]    functional
    Switch Connection    OperationManager
    ${output}=    Read Until Regexp    Dispatching command: Command\[type: 1302, id: [0-9]*, source: 2,
    Log    ${output}
