*** Settings ***
Documentation    Execute AZ_DRIVE_RESET command and get Ack/Error responses.
Library    SSHLibrary
Library    String
...
*** Test Cases ***
Send Command
    [Documentation]    Send the AZ_DRIVE_RESET command.
    [Tags]    functional
...
Verify Operation Manager Dispatches Command
    [Documentation]    Verify that Operation Manager dispatches the AZ_DRIVE_RESET command.
    [Tags]    functional
    Switch Connection    OperationManager
    ${output}=    Read Until Regexp    Dispatching command: Command\[type: 201, id: [0-9]*, source: 2,
    Log    ${output}
...
Verify PXI Message
    ...
    Should Match Regexp    ${output}    [1-9][0-9]*-[1-9][0-9]*-201-2-[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}-4
