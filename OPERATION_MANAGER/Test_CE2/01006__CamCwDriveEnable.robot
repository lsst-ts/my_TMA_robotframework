*** Settings ***
Documentation    Execute CAM_CW_DRIVE_ENABLE command and get Ack/Error responses.
Library    SSHLibrary
Library    String
...
*** Test Cases ***
Send Command
    [Documentation]    Send the CAM_CW_DRIVE_ENABLE command.
    [Tags]    functional
...
Verify Operation Manager Dispatches Command
    [Documentation]    Verify that Operation Manager dispatches the CAM_CW_DRIVE_ENABLE command.
    [Tags]    functional
    Switch Connection    OperationManager
    ${output}=    Read Until Regexp    Dispatching command: Command\[type: 1006, id: [0-9]*, source: 2,
    Log    ${output}
...
Verify PXI Message
    ...
    Should Match Regexp    ${output}    [1-9][0-9]*-[1-9][0-9]*-1006-2-[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}-2-0
