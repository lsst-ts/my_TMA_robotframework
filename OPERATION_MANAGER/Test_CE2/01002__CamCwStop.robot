*** Settings ***
Documentation    Execute CAM_CW_STOP command and get Ack/Error responses.
Library    SSHLibrary
Library    String
...
*** Test Cases ***
Send Command
    [Documentation]    Send the CAM_CW_STOP command.
    [Tags]    functional
...
Verify Operation Manager Dispatches Command
    [Documentation]    Verify that Operation Manager dispatches the CAM_CW_STOP command.
    [Tags]    functional
    Switch Connection    OperationManager
    ${output}=    Read Until Regexp    Dispatching command: Command\[type: 1002, id: [0-9]*, source: 2,
    Log    ${output}
