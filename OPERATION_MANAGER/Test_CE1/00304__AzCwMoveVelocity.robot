*** Settings ***
Documentation    Execute AZ_CW_MOVE_VELOCITY command and get Ack/Done responses.
Library    SSHLibrary
Library    String
...
*** Test Cases ***
Send Command
    [Documentation]    Send the AZ_CW_MOVE_VELOCITY command.
    [Tags]    functional
...
Verify Operation Manager Dispatches Command
    [Documentation]    Verify that Operation Manager dispatches the AZ_CW_MOVE_VELOCITY command.
    [Tags]    functional
    Switch Connection    OperationManager
    ${output}=    Read Until Regexp    Dispatching command: Command\[type: 304, id: [0-9]*, source: 2,
    Log    ${output}
...
Verify PXI Message
    ...
    Should Match Regexp    ${output}    [1-9][0-9]*-[1-9][0-9]*-304-2-[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}-8\\.151980-1\\.802377-40\\.582083
...
Send Stop Command
    [Documentation]    Send stop for the AZ_CW_MOVE_VELOCITY command.
    [Tags]    functional
    Switch Connection    Eui
    Comment    Enter command id
    Write    302
